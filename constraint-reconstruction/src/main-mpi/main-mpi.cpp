...
void SART(const Point3DF& origin, MrcStackM& projs, std::vector<float*> pslices, Volume& vol, Coeff coeffv[], int iteration, float gamma)
{
...
int ATOM(Particle& particle, const char* method, int myid, int procs){
	...
	if(strcmp("SART", method) == 0){
		SART(origin, projs, pslices, vol, &params[0], 1, particle.gamma);
	}
	else{
