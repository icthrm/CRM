...
inline void DrawLine(IplImage* img, const util::_point& pt1, const util::_point& pt2)
{
    CvScalar color = CV_RGB(255, 255, 255);
    cvLine(img, cvPoint(pt1.x, pt1.y), cvPoint(pt2.x, pt2.y), color);
}
...
inline void ConvertTo1(IplImage* img, float minimum, float maximum)
{
...
inline void ConvertTo1(const IplImage* img, float average, float deviation, float minimum, float maximum, bool cutoff = false)
{
#define CUTOFF (3)//(4)//(5)
...
inline void GetImgAvgSdvCutoffMinMax(const IplImage* img, double* average, double* deviation, double* minimum, double* maximum, double cutoff = 3)
{
	CvScalar mean, sdv;
    cvAvgSdv(img, &mean, &sdv);
    cvMinMaxLoc(img, minimum, maximum);
...
inline void CopyToUChar256(const IplImage* img, IplImage** cpy, int w, int b, double average, double deviation, double minimum, double maximum, bool cutoff)
{
#define CUTOFF (3)//(4)//(5)
